...
def ensure_one_servicerating():
    ...
    cursor.execute("DELETE FROM servicerating_response WHERE contact_id NOT IN \
    (SELECT MAX(id) as id FROM servicerating_contact GROUP BY key)")
    affected = cursor.rowcount
    vumi_fire_metric.delay(metric="servicerating.duplicates", value=affected, agg="last")
    return affected
